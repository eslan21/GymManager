...
export function Sidebar({ isOpen, setIsOpen }) {
  ...
  const navItems = [
    { name: 'Inicio', href: '/admindashboard/', icon: <ChartBarIcon className="h-6 w-6" /> },
    { name: 'Check-In', href: '/admindashboard/checkinuser/', icon: <UsersIcon className="h-6 w-6" /> },
    { name: 'Asistencia Diaria', href: '/admindashboard/attendance/', icon: <ClipboardDocumentCheckIcon className="h-6 w-6" /> },
  ];
      ...
      >
        ...
        <nav className="flex-1 px-3 py-4 space-y-2">
          {navItems.map((item) => (
            ...
            >
              {item.icon}
              <span
                className={`ml-4 font-medium whitespace-nowrap transition-opacity ${isOpen ? 'opacity-100' : 'md:opacity-0 md:hidden'
                  }`}
              >
                {item.name}
              </span>
        ...
        <div className="p-3 border-t border-gray-700">
          <button onClick={sessionEnded} className="flex items-center p-3 rounded-lg hover:bg-red-600 group">
            ...
            <span className={`ml-4 font-medium whitespace-nowrap transition-opacity ${isOpen ? 'opacity-100' : 'md:opacity-0 md:hidden'
              }`}>
              Cerrar Sesión
            </span>
          </button>
